...
Then we must make sure that the protocol behaves well in the presence of a network adversary. So, we have to investigate TLC's resilience to adversaries within our thread model. We may violate our thread model to make sure that we can't make progress. 
...
E-mail protocols have been used for the second main implementation.  SMTP is used for sending e-mails and IMAP used for receiving e-mails. E-mail has been chosen because it has a high delay so we could test TLC in high-delay environments. 

An e-mail server has been set up in a virtual machine, and it was not powerful. The server introduced some delays to our simulations because it couldn't deliver messages quickly. Since the goal was to test TLC's delay tolerance, the delay caused by the server was not a major problem.
...
In this experiment, we simulated TLC with majority thresholds with several numbers of nodes. we start with 11 nodes and end up with 101 nodes, in 10 increments. All nodes are running on the same machine, communicating through the localhost, using libp2p-go, and go-gossipsub packages.
...
It's worth mentioning that messages are encoded using Protobuf and sent in the subject field of e-mails, in order to prevent problems caused by UTF-8 encoding of e-mail body in the SMTP protocol. 
...
For simulating this attack, we have used 11 nodes with the majority threshold and simulated for 5 timesteps. First, we start with all nodes in a good state and then the attacker isolates some nodes to prevent them from receiving and sending messages.
...
In this case, a sudden network split has been simulated while some nodes are under attack. For this execution, we have delayed some messages between 2 groups of nodes for a limited time, while a minority of nodes are also under attack from an adversary. The number of the nodes and thresholds are identical to previous attacks.
